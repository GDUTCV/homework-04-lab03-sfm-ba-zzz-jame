...
def detect_keypoints(image_file: os.path):
    ...
    keypoints, descriptors = sift.detectAndCompute(cv2.imread(image_file), None)
...
def create_feature_matches(image_file1: os.path, image_file2: os.path, lowe_ratio: float = 0.6, min_matches: int = 10):
    ...
    # 使用knnMatch找到每个关键点的两个最佳匹配
    matches = bf.knnMatch(descriptors1, descriptors2, k=2)

    # 应用Lowe's比率测试来筛选良好的匹配
    good_matches = []
    for m, n in matches:
...
def create_scene_graph(image_files: list, min_num_inliers: int = 40):
    ...
    <min_num_inliers> 
    """
    # 创建场景图，添加边的条件是两图像间的几何验证内点数至少为 min_num_inliers
    for i, id_1 in enumerate(image_ids):
        for j, id_2 in enumerate(image_ids[i + 1:], start=i + 1):  # 从 i+1 开始，避免重复和自环
            match_id = f"{id_1}_{id_2}"  # 使用 f-string 格式化字符串
            match_save_file = os.path.join(RANSAC_MATCH_DIR, f"{match_id}.npy")  # 使用 f-string

            # 检查匹配文件是否存在
            if os.path.exists(match_save_file):
                inliers = np.load(match_save_file)
                if len(inliers) >= min_num_inliers:  # 使用 >= 确保至少有 min_num_inliers 个内点
                    graph.add_edge(i, j)  # 添加边到图
